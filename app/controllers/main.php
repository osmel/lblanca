		...
		if ( $this->form_validation->run() == FALSE ){
			echo validation_errors('<span class="error">','</span>');
		        ...
		        if ( $usuario_check != FALSE ){
						...
						$this->email->from($desde,'Línea Blanca');
						$this->email->to($correo_enviar);
						$this->email->subject('Recuperación de contraseña de Línea Blanca');
				...
				if ($this->input->post( 'pass_1' ) === $this->input->post( 'pass_2' ) ){
					...
					if ( $login_check != FALSE ){		
						...
						if ( $guardar !== FALSE ){
									...
									$this->email->subject('Has sido dado de alta en Línea blanca');
